...
namespace logstore_last_updated_course_module\log;
...
class store implements \tool_log\log\writer, \core\log\sql_reader {
    ...
    protected function is_event_ignored(\core\event\base $event) {
        if (isguestuser()
                || (!$event instanceof \core\event\course_module_created && !$event instanceof \core\event\course_module_updated)) {
            return true;
        }
        return false;
    }
    ...
    protected function insert_event_entries($evententries) {
        ...
        if (!\core\session\manager::is_loggedinas()) {
            foreach ($evententries as $evententry) {
                ...
                $datas->lasttimeupdated = $evententry['timecreated'];
                if ($record = $DB->get_record('logstore_lastupdated_log',
                        array('cmid' => $datas->cmid))) {
                    $datas->id = $record->id;
                    $DB->update_record('logstore_lastupdated_log', $datas);
                } else {
                    $DB->insert_record('logstore_lastupdated_log', $datas);
                }
            }
        }
    }
    ...
    public function get_events_select($selectwhere, array $params, $sort, $limitfrom, $limitnum) {
        ...
        $events = array();
        $records = $DB->get_recordset_select('logstore_lastupdated_log', $selectwhere, $params, $sort, '*', $limitfrom, $limitnum);
    ...
    public function get_events_select_iterator($selectwhere, array $params, $sort, $limitfrom, $limitnum) {
        ...
        $recordset = $DB->get_recordset_select(
                'logstore_lastupdated_log', $selectwhere, $params, $sort, '*', $limitfrom, $limitnum
        );
    ...
    public function get_events_select_count($selectwhere, array $params) {
        global $DB;
        return $DB->count_records_select('logstore_lastupdated_log', $selectwhere, $params);
    }
