...
namespace logstore_last_updated_course_module\task;
...
class cleanup_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('taskcleanup', 'logstore_last_updated_course_module');
    }
    ...
    public function execute() {
        ...
        $loglifetime = (int)get_config('logstore_last_updated_course_module', 'loglifetime');
        ...
        while ($min = $DB->get_field_select("logstore_lastupdated_log",
                        "MIN(lasttimeupdated)", "lasttimeupdated < ?", $lifetimep)
        ) {
            ...
            $DB->delete_records_select("logstore_lastupdated_log", "lasttimeupdated < ?", $params);
            if (time() > $start + 300) {
